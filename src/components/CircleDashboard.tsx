          ...
          {members.map((member, idx) => (
            <div key={member.id} className="text-center">
              <div className="w-12 h-12 mx-auto rounded-full flex items-center justify-center text-sm font-medium mb-2 bg-green-100 text-green-700 ring-2 ring-green-200">
                {member.full_name ? member.full_name.split(' ').map(n => n[0]).join('').toUpperCase() : member.id.slice(0, 2).toUpperCase()}
              </div>
              <p className="text-xs text-gray-600 truncate">{member.full_name || member.id}</p>
            </div>
          ))}
        </div>
      </div>
